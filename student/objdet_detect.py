...
def create_model(configs):
    ...
    if (configs.arch == 'darknet') and (configs.cfgfile is not None):
        ...
        model = get_pose_net(num_layers=configs.num_layers,
                             heads=configs.heads,
...
def detect_objects(input_bev_maps, model, configs):
    ...
    with torch.no_grad():  
        ...
        if 'darknet' in configs.arch:
            ...
            detections = detections[0][1].tolist()

    # Extract 3d bounding boxes from model response
    objects = []

    for detection in detections:
        # detection: [score, x, y, z, h, w, l, yaw]
        _, _x, _y, _z, _h, _w, _l, _yaw, = detection
        x_boundary = configs.boundary['maxX'] - configs.boundary['minX']
        y_boundary = configs.boundary['maxY'] - configs.boundary['minY']
        x = _y / configs.img_size * x_boundary + configs.boundary['minX']
        y = _x / configs.img_size * y_boundary + configs.boundary['minY']
        z = _z + configs.boundary['minZ']
        w = _w / configs.img_size * y_boundary
        l = _l / configs.img_size * x_boundary
        yaw = -_yaw
